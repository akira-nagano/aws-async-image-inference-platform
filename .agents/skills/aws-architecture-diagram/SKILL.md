...
2. Run the project-local post-processing, validation, and preview pipeline:

   ```bash
   mise exec -- uv run ./.agents/skills/aws-architecture-diagram/scripts/validate_drawio_bundle.py ./docs/<filename>.drawio
   ```

3. If validation fails, fix errors, rewrite the file, and rerun the same command
4. If export format requested, run draw.io CLI (see `references/cli-export.md`)
...
5. Run the project-local validation pipeline:

   ```bash
   mise exec -- uv run ./.agents/skills/aws-architecture-diagram/scripts/validate_drawio_bundle.py ./docs/<filename>.drawio
   ```

6. The command applies deterministic fixers, validates structure and AWS shapes, and prints the browser preview URL only after validation passes
7. If validation fails, fix the errors, rewrite the file, and rerun the command
